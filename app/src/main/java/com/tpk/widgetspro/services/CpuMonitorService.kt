...
class CpuMonitorService : BaseMonitorService() {
    ...
    private val prefListener = SharedPreferences.OnSharedPreferenceChangeListener { _, key ->
        if (key == "cpu_interval" && ::cpuMonitor.isInitialized) {
            val newInterval = prefs.getInt("cpu_interval", 60).coerceAtLeast(1)
            cpuMonitor.startMonitoring(newInterval)
        }
    }

    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        useRoot = intent?.getBooleanExtra("use_root", false) == true

        if (useRoot && !PermissionUtils.hasRootAccess()) {
            stopSelf()
            return START_NOT_STICKY
        } else if (!useRoot && !PermissionUtils.hasShizukuPermission()) {
            stopSelf()
            return START_NOT_STICKY
        }
            ...
            appWidgetIds.forEach { appWidgetId ->
                val views = RemoteViews(packageName, R.layout.cpu_widget_layout).apply {
                    ...
                    setImageViewBitmap(
                        ...
                        createGraphBitmap(this@CpuMonitorService, dataPoints, DottedGraphView::class)
                    )
                }
    ...
    private fun <T : BaseDottedGraphView> createGraphBitmap(
        context: Context,
        data: Any,
        viewClass: KClass<T>
    ): Bitmap {
